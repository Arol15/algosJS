...
// and the reversed string must contain the same whitespaces
//as the original string. For example, given the string
// "whitespaces
// 4" you would be expected to return "4
// whitespaces".
...
function reverseWords(string) {
  ...
  for (let i = string.length - 1; i >= 0; i--) {
    if (string[i] === " ") {
      result.push(string.slice(i + 1, startIndex));
      startIndex = i + 1;
    } else if (i === 0) {
      result.push(string.slice(i, startIndex));
    }
    // console.log(result);
  }
...
console.log(reverseWords("AlgoExpert is    the best!")); //"best! the is AlgoExpert"
console.log(reverseWords("whitespaces 4"));
